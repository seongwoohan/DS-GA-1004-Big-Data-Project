# spark-submit SixthCol1.py /user/ly976/NYPD_Complaint_Data_Historic.csv SixthCol1.txt                                                                                  
# hfs -getmerge SixthCol1.txt SixthCol1.txt                                                                                                                             

#check whether the report date is valid and after occur start date
import sys
import os
...
def isValidDateTime(input):
    correctDate = None
    if input[1] == "":
        month2, day2, year2 = input[5].split('/')
        try:
...
def fun(input):
    if input[5] == "":
        ...
        return "%s\tDate\tReport\tVALID" % (input[5])
    else:
        return "%s\tDate\tReport\tINVALID\t%s\tDate\tOccur start" % (input[5], input[1])
